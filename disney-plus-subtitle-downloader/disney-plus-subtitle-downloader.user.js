...
// @updateURL   https://raw.githubusercontent.com/fabiorzfreitas/userscripts/main/disney-plus-subtitle-downloader/disney-plus-subtitle-downloader.user.js
// @downloadURL https://raw.githubusercontent.com/fabiorzfreitas/userscripts/main/disney-plus-subtitle-downloader/disney-plus-subtitle-downloader.user.js
// @grant       none
...
(function() {
        ...
        switch (msg.type) {
          ...
          case 'dplus_sd_m3u8': {
            ...
            if (content && url && content.includes('TYPE=SUBTITLES')) {
              ...
              debuglog('M3U8 fetched: ' + url.substring(0, 80));
              processM3U8(content);
            }
            break;
          }
          ...
          case 'dplus_sd_playback': {
            ...
            if (headers && typeof headers === 'object') {
              ...
              debuglog('Playback headers fetched: ' + keys.length + ' total, x-headers: ' + xKeys.length + ' (' + xKeys.join(', ') + ')');
            ...
            if (body && !AppState._bodyCaptured) {
              try {
                ...
                debuglog('Playback body fetched, keys: ' + Object.keys(AppState.playbackBody).join(', '));
              } catch {}
            }
            break;
          }
          ...
          case 'dplus_sd_season': {
            ...
            } catch(e) { debuglog('Season data parse error: ' + e.message); }
            break;
          }
          ...
          case 'dplus_sd_episode_info': {
            ...
            } catch(e) { debuglog('Episode info parse error: ' + e.message); }
            break;
          }
  ...
  async function fetchEpisodeM3U8(episode) {
    ...
    if (!AppState.authToken) {
      throw new Error('Auth token not found. Please play an episode and try again.');
    }
    if ((Date.now() - AppState.authTokenTime) > TIMING.TOKEN_MAX_AGE) {
      throw new Error('Auth token has expired. Please play an episode and refresh.');
    }
    ...
    debuglog('Playback request: ' + episode.contentId.substring(0, 16) + ' | x-headers: ' + xHeaderKeys.length + ' (' + xHeaderKeys.join(', ') + ')');
    ...
    if (proxyResult.status === 401 || proxyResult.status === 403) {
      ...
      throw new Error(`Token invalid (${proxyResult.status}). Please play an episode and refresh.`);
    }
    if (!proxyResult.ok) {
      throw new Error(`Playback API error: ${proxyResult.status}`);
    }

    let playbackData;
    try { playbackData = JSON.parse(proxyResult.body); }
    catch { throw new Error('Playback response parsing error'); }
  ...
  function parseExploreApiResponse(obj) {
    ...
    for (const s of episodeContainer.seasons) {
      ...
      // Extract season number: try direct fields, then parse from visuals.name ("1. Season" → 1)
  ...
  function processSeasonData(apiResponse) {
    ...
    debuglog(`Season data: ${AppState.seasonData.seriesTitle || '?'} — ${AppState.seasonData.seasons.length} season, ${totalEps} episode`);
    createMenu();
  ...
  async function fetchRemainingEpisodesViaApi() {
    ...
    try {
      for (const season of AppState.seasonData.seasons) {
        ...
        if (!season.seasonId) {
          debuglog(`Season ${season.seasonNumber}: seasonId not found, API fetch failed`);
          season._fetchCompleted = true;
          continue;
        }

        debuglog(`Season ${season.seasonNumber}: ${season.episodes.length} episodes exist, the rest is being retrieved from the API (seasonId: ${season.seasonId})...`);
        ...
        try {
          ...
          while (hasMore && fetchCount < 20) {
            ...
            if (!resp.ok) {
              debuglog(`Season API error: ${resp.status}`);
              break;
            }
            ...
            debuglog(`Season ${season.seasonNumber} offset=${offset}: ${pageEps.length} episodes found`);
          ...
          season._fetchCompleted = true;
          debuglog(`Season ${season.seasonNumber}: total ${season.episodes.length} episodes (API fetch completed)`);
        } catch (e) {
          debuglog(`Season ${season.seasonNumber} API fetch error: ${e.message}`);
          season._fetchCompleted = true;
        }
      }
      ...
      const totalEps = AppState.seasonData.seasons.reduce((s, se) => s + se.episodes.length, 0);
      debuglog(`API fetch completed: total ${totalEps} episodes`);
      createMenu();
  ...
  function processEpisodeInfo(data) {
    ...
    if (titleMatch) {
      ...
      // Episode title from subtitle field: "S1:B1 The Yankee Dodge" → "The Yankee Dodge"
      let episodeTitle = '';
  ...
  async function fetchSeriesPageFromEpisodeInfo(rawData) {
    ...
    if (!AppState.authToken) {
      debuglog('Series page: Auth token not found, pending...');
      return;
    }
    ...
    try {
      ...
      if (!urlMediaId) {
        debuglog('Series page: URL mediaId not found');
        return;
      }
      ...
      debuglog('Deeplink API query: ' + urlMediaId);
      ...
      if (!dlResp.ok || !dlResp.body) {
        debuglog('Deeplink API error: ' + (dlResp.status || 'no response'));
        return;
      }
      ...
      if (!seriesPageId) {
        debuglog('Series entity ID not found (deeplink actions: ' + actions.length + ')');
        return;
      }

      debuglog('Series entity ID found: ' + seriesPageId);
      ...
      if (resp.ok && resp.body) {
        ...
        debuglog('Series page error: HTTP ' + (resp.status || '?'));
      }
    } catch (e) {
      debuglog('Series page error: ' + e.message);
    }
  }
  ...
  function processM3U8(content) {
    ...
    if (tracks.length > 0) {
      ...
      debuglog(`${tracks.length} subtitle track found`);
  ...
  async function proactiveFetchEpisodeInfo() {
    ...
    loadPersistedState();
    if (!AppState.authToken) {
      debuglog('Proactive PE: no auth token, pending...');
      ...
      if (!AppState.authToken) {
        debuglog('Proactive PE: auth token not found, aborted');
        return;
      }
    }
    ...
    if (!peUrl) {
      debuglog('Proactive PE: playerExperience URL not found');
      ...
      if (urlMediaId) {
        ...
        debuglog('Proactive PE: URL mediaId exists (' + urlMediaId.substring(0, 8) + ') but availId is unknown');
      }
      return;
    }
    ...
    try {
      ...
      if (resp.ok && resp.body) {
        ...
        processEpisodeInfo(data);
        debuglog('Proactive PE successful!');
      } else {
        debuglog('Proactive PE error: HTTP ' + resp.status);
      }
    } catch (e) {
      debuglog('Proactive PE fetch error: ' + e.message);
    }
  }
  ...
  async function downloadSingleTrack(track, format, zipInstance) {
    ...
    if (vttUrls.length === 0) throw new Error('VTT segment not found');
    ...
    if (segmentTexts.length === 0) throw new Error('Segment could not be downloaded');

    const mergedVTT = mergeVTTSegments(segmentTexts);
    const content = fmt === 'srt' ? vttToSrt(mergedVTT) : 'WEBVTT\n\n' + mergedVTT;
    if (!content || content.trim().length < 10) throw new Error('Empty content');
  ...
  async function downloadAllAsZip() {
    ...
    for (let i = 0; i < AppState.subtitleTracks.length; i++) {
      ...
      try {
        ...
        debuglog(`Error (${track.LANGUAGE}): ${e.message}`);
      }
  ...
  async function downloadSeasonSubtitles(seasonNumber, langCode, format) {
    ...
    if (season.seasonId && season.hasMore && !season._fetchCompleted) {
      try {
        ...
        let fetchCount = 0;
        debuglog(`Season ${seasonNumber}: Retrieving remaining episodes from API (seasonId: ${season.seasonId}, available: ${season.episodes.length})...`);
        ...
        while (hasMorePages && fetchCount < 20) {
          ...
          if (!pageResp.ok) {
            debuglog(`Season API offset=${offset}: HTTP ${pageResp.status}`);
            break;
          }
          ...
          debuglog(`Season ${seasonNumber} offset=${offset}: ${pageEps.length} episodes`);
        ...
        season.hasMore = false;
        season._fetchCompleted = true;
        debuglog(`Season ${seasonNumber}: total ${season.episodes.length} episodes (pre-download fetch completed)`);
      } catch (e) {
        debuglog(`Season API fetch error: ${e.message}`);
      }
    }
    ...
    debuglog(`Season download is starting: ${episodes.length} episode, language: ${langCode}`);
    for (let i = 0; i < 3 && i < episodes.length; i++) {
      const ep = episodes[i];
      debuglog(`  Episode[${i}]: contentId=${ep.contentId?.substring(0, 16)}, ep=${ep.episodeNumber}, s=${ep.seasonNumber}, title="${ep.title}", resourceId=${ep.resourceId ? 'var' : 'none'}`);
    }
    ...
    for (let i = 0; i < episodes.length; i++) {
      ...
      try {
        ...
        if (!targetTrack) { debuglog(`[${epLabel}] '${langCode}' not found`); continue; }
        ...
        debuglog(`[${epLabel}] Error: ${e.message}`);
    ...
    if (successCount > 0) {
      ...
      const zipContent = await zip.generateAsync({ type: 'blob' });
      saveAs(zipContent, zipName);
      debuglog(`ZIP downloaded: ${zipName} (${successCount}/${episodes.length})`);
    }
  ...
  function updateProgress(pct, text, show = true) {
    ...
    if (btn) {
      if (show && AppState.isProcessing) {
        ...
        btn.textContent = match ? `Downloading ${match[1]}/${match[2]}` : (text || 'Downloading...');
      } else if (!show) {
        const count = AppState.subtitleTracks?.length || 0;
        btn.innerHTML = `<span>Subtitles</span>${count > 0 ? `<span class="sd-badge">${count}</span>` : ''}`;
      }
    }
  ...
  function createMenu() {
    ...
    if (AppState.isProcessing && AppState.progressText) {
      ...
      btn.textContent = match ? `Downloading ${match[1]}/${match[2]}` : AppState.progressText;
    } else if (AppState.isProcessing) {
      btn.textContent = 'Downloading...';
    } else {
      btn.innerHTML = `<span>Subtitles</span>${tracks.length > 0 ? `<span class="sd-badge">${tracks.length}</span>` : ''}`;
    }
    ...
    if (isPlayPage && normalTracks.length > 0) {
      ...
      section.innerHTML = `<div class="sd-section-title">Subtitles (${normalTracks.length})</div>`;
      for (const track of normalTracks) {
        ...
        row.querySelector('button').addEventListener('click', async () => {
          try {
            showToast(`${getLangName(track.LANGUAGE)} downloading...`);
            await downloadSingleTrack(track, AppState.format);
            showToast(`${getLangName(track.LANGUAGE)} downloaded`, 'success');
    ...
    if (isPlayPage && forcedTracks.length > 0) {
      const section = document.createElement('div');
      section.className = 'sd-section';
      section.innerHTML = `<div class="sd-section-title">Forced Subtitles (${forcedTracks.length})</div>`;
    ...
    if (isPlayPage && tracks.length > 0) {
      ...
      allBtn.textContent = `Download all ZIP (${tracks.length})`;
    ...
    if (AppState.seasonData && AppState.seasonData.seasons.length > 0) {
      ...
      seasonSection.innerHTML = `<div class="sd-section-title">Season Download${seasonTitle ? ' — ' + seasonTitle : ''}</div>`;
      ...
      for (const s of AppState.seasonData.seasons) {
        ...
        opt.textContent = `Season ${s.seasonNumber} (${s.episodes.length} episodes)`;
      ...
      dlBtn.textContent = 'Download Whole Season';
  ...
  function init() {
    ...
    debuglog('Disney+ Subtitle Downloader v' + VERSION + ' launched');
  }
